...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([getTrendingMovies(), searchMovies("best movies 2025")])
      .then(([t, f]) => {
        setTrending(t.items?.slice(0, 10) || []);
        setFeatured(f.items?.slice(0, 10) || []);
      })
  ...
  const Skeleton = () => (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
      {Array.from({ length: 10 }).map((_, i) => (
        <div
          key={i}
          className="rounded-xl overflow-hidden bg-[hsl(220,18%,11%)] animate-pulse"
        >
          <div className="aspect-video bg-[hsl(220,15%,16%)]" />
          <div className="p-3 space-y-2">
            <div className="h-3 bg-[hsl(220,15%,16%)] rounded w-full" />
            <div className="h-3 bg-[hsl(220,15%,16%)] rounded w-2/3" />
          </div>
        </div>
      ))}
    </div>
      ...
      <section className="relative pt-24 pb-16 px-4 overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-lg text-[hsl(220,10%,55%)] mb-8 max-w-xl mx-auto">
            Search and stream thousands of movies, trailers and clips — all in
            one place.
          </p>
          <form onSubmit={handleSearch} className="max-w-xl mx-auto">
            <div className="relative flex items-center">
              ...
              <button
                type="submit"
                className="absolute right-2 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-xl px-5 py-2.5 transition-all text-sm"
              >
                Search
              </button>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-16">
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-5">
            ...
            <button
              onClick={() => router.push("/search?q=trending+movies+2025")}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {trending.map((v) => (
                <MovieCard key={v.id.videoId} video={v} />
              ))}
            </div>
          )}
        </section>
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-5">
            ...
            <button
              onClick={() => router.push("/search?q=best+movies+2025")}
              className="flex items-center gap-1 text-sm text-orange-500 hover:text-orange-300 transition-colors"
            >
              See all <ChevronRight className="w-4 h-4" />
            </button>
